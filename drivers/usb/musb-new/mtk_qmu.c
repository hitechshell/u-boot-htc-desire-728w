...
u32 qmu_used_gpd_count(u8 isRx, u32 num)
{
	int ret;
	if (isRx) {
		ret = (Rx_gpd_max_count[num] - 1) - Rx_gpd_free_count[num];
	} else {
		ret = (Tx_gpd_max_count[num] - 1) - Tx_gpd_free_count[num];
	}
	return ret;
}
...
{	int ret;
	if (isRx) {
		ret = Rx_gpd_free_count[num];
	} else {
		ret = Tx_gpd_free_count[num];
	}
	return ret;
}
...
int qmu_init_gpd_pool(struct device *dev)
{
	...
	for (i = 1; i <= RXQ_NUM; i++) {
		...
		Rx_gpd_head[i] = ptr;
		pr_info("ALLOC RX GPD Head [%d] Virtual Mem=%p, DMA addr=%p\n", i, Rx_gpd_head[i],
			 io_ptr);
	...
	for (i = 1; i <= TXQ_NUM; i++) {
		...
		Tx_gpd_head[i] = ptr;
		pr_info("ALLOC TX GPD Head [%d] Virtual Mem=%p, DMA addr=%p\n", i, Tx_gpd_head[i],
			 io_ptr);
...
/* TGPD_CLR_FORMAT_BPS(gpd); */
	...
	pr_info("[RX]" "Rx_gpd_end[%d]=%p gpd=%p\n", ep_num, Rx_gpd_end[ep_num], gpd);
...
static void prepare_tx_gpd(dma_addr_t *pBuf, u32 data_len, u8 ep_num, u8 zlp, u8 isioc)
{
	...
	pr_info("[TX]" "Tx_gpd_end[%d]=%p gpd=%p\n", ep_num, Tx_gpd_end[ep_num], gpd);
...
{
	pr_info("mtk_qmu_insert_task ep_num: %d, isRx: %d, buf: %p, length: %d zlp: %d isioc: %d\n",
...
void qmu_done_rx(struct musb *musb, u8 ep_num)
{
	...
	if (!req) {
		pr_err("[RXD]" "%s Cannot get next request of %d, but QMU has done.\n",
				__func__, ep_num);
		return;
	}
	request = &req->request;
	...
	pr_info("[RXD]" "%s EP%d, Last=%p, Current=%p, End=%p\n",
	...
	if (gpd == gpd_current) {
		...
		pr_err("[RXD][ERROR]EP%d RQCSR=%x, RQSAR=%x, RQCPR=%x, RQLDPR=%x\n",
			ep_num,
			MGC_ReadQMU32(base, MGC_O_QMU_RQCSR(ep_num)),
		...
		pr_err("[RXD][ERROR]QCR0=%x, QCR2=%x, QCR3=%x, QGCSR=%x\n",
		...
		pr_err("[RXD][ERROR]HWO=%d, Next_GPD=%p ,DataBufLen=%d, DataBuf=%p, RecvLen=%d, Endpoint=%d\n",
	...
	while (gpd != gpd_current && !TGPD_IS_FLAGS_HWO(gpd)) {
		...
		if (rcv_len > buf_len)
			pr_err("[RXD][ERROR] rcv(%d) > buf(%d) AUK!?\n", rcv_len, buf_len);

		pr_info("[RXD]" "gpd=%p ->HWO=%d, Next_GPD=%p, RcvLen=%d, BufLen=%d, pBuf=%p\n",
	...
	pr_info("[RXD]" "%s EP%d, Last=%p, End=%p, complete\n", __func__,
...
void qmu_done_tx(struct musb *musb, u8 ep_num)
{
	...
	pr_info("[TXD]" "%s EP%d, Last=%p, Current=%p, End=%p\n",
		 __func__, ep_num, gpd, gpd_current, Tx_gpd_end[ep_num]);

	/*gpd_current should at least point to the next GPD to the previous last one. */
	if (gpd == gpd_current) {
		pr_info("[TXD] gpd(%p) == gpd_current(%p)\n", gpd, gpd_current);
		return;
	}
	...
	while (gpd != gpd_current && !TGPD_IS_FLAGS_HWO(gpd)) {
		...
		pr_info("[TXD]" "gpd=%p ->HWO=%d, BPD=%d, Next_GPD=%p, DataBuffer=%p, BufferLen=%d request=%p\n",
		...
		req = next_request(musb_ep);
		if (!req) {
			pr_err("[TXD]" "%s Cannot get next request of %d, but QMU has done.\n",
					__func__, ep_num);
			return;
		}
		request = &req->request;
	...
	if (gpd != gpd_current && TGPD_IS_FLAGS_HWO(gpd)) {
		...
		pr_err("[TXD][ERROR]" "HWO=%d, BPD=%d, Next_GPD=%p, DataBuffer=%p, BufferLen=%d, Endpoint=%d\n",
			(u32) TGPD_GET_FLAG(gpd), (u32) TGPD_GET_FORMAT(gpd),
			TGPD_GET_NEXT(gpd), TGPD_GET_DATA(gpd),
			(u32) TGPD_GET_BUF_LEN(gpd), (u32) TGPD_GET_EPaddr(gpd));
	}

	pr_info("[TXD]" "%s EP%d, Last=%p, End=%p, complete\n", __func__,
	...
	if (req != NULL) {
		if (request->length == 0) {
			...
			pr_warn("[TXD]" "==Send ZLP== %p\n", req);
			musb_tx_zlp_qmu(musb, req->epnum);

			pr_warn("[TXD]" "Giveback ZLP of EP%d, actual:%d, length:%d %p\n",
...
void h_qmu_done_rx(struct musb *musb, u8 ep_num)
{
	...
	if (gpd == gpd_current) {
		...
		pr_err("[RX]HWO=%d, Next_GPD=%p ,BufLen=%d, Buf=%p, RLen=%d, EP=%d\n",
				(u32)TGPD_GET_FLAG(gpd), TGPD_GET_NEXT(gpd),
				(u32)TGPD_GET_DataBUF_LEN(gpd), TGPD_GET_DATA(gpd),
				(u32)TGPD_GET_BUF_LEN(gpd), (u32)TGPD_GET_EPaddr(gpd));
	...
	if (!gpd || !gpd_current) {
		pr_err("[RXD][ERROR] EP%d, gpd=%p, gpd_current=%p, ishwo=%d, rx_gpd_last=%p, RQCPR=0x%x\n",
	...
	while (gpd != gpd_current && !TGPD_IS_FLAGS_HWO(gpd)) {
		...
		if (done) {
			...
			if (!hw_ep->in_qh) {
				pr_debug("hw_ep:%d, QH NULL after advance_schedule\n", ep_num);
				return;
			}
		}
	}
...
void h_qmu_done_tx(struct musb *musb, u8 ep_num)
{
	...
	/* NORMAL EXEC FLOW */
	while (gpd != gpd_current && !TGPD_IS_FLAGS_HWO(gpd)) {
		pr_info("[TXD]""gpd=%p ->HWO=%d, BPD=%d, Next_GPD=%p, DataBuffer=%p, BufferLen=%d\n",
			gpd, (u32)TGPD_GET_FLAG(gpd), (u32)TGPD_GET_FORMAT(gpd),
...
void mtk_qmu_host_rx_err(struct musb *musb, u8 epnum)
{
	...
	pr_debug("<== hw %d rxcsr %04x, urb actual %d\n",
	...
	if (done)
		pr_debug("FIXME!!!, to be implemented, related HW/SW abort procedure\n");

finished:
	{
		printf("USB20_HOST, RXQ<%d> ERR, CSR:0x%x\n", epnum, val);
	}
}
...
finished:
	{
		printf("USB20_HOST, TXQ<%d> ERR, CSR:0x%x", epnum, val);
	}
		...
		if (request->request.dma != DMA_ADDR_INVALID) {
			if (request->tx) {
				...
				if (request->request.length > 0) {
					...
					pr_err("[TX]" "Send ZLP cases, may be a problem!!!\n");
					musb_tx_zlp_qmu(musb, request->epnum);
					musb_g_giveback(musb_ep, &(request->request), 0);
				} else {
					pr_err("ERR, TX, request->request.length(%d)\n",
						request->request.length);
				}
			} else {
